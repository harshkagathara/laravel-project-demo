...
<div class="row" >
	<div class="col-12">
		...
		<div class="card " style="margin-top: 10px;">
			<!-- <div class="card-header" style="background:#8141a1;">
				<h3 class="card-title" style="color: white;">Add Dish</h3>
			</div> -->
			<!-- /.card-header -->
			<!-- form start -->
			<form role="form" id="createForm" method="POST" action="{{route('save.dish')}}" enctype="multipart/form-data">
                @csrf
            <div class="card-body">
    ...
    <select name=addons_category_id class=" form-control select2" style="width: 100%;"   >
        @foreach($addonscategories as $addonscategory) 
            <option value="{{$addonscategory->id}}" >{{$addonscategory->name}}</option>
        @endforeach
            </select>
</div>
</div>
